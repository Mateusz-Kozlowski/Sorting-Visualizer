...
void App::updateSpeedModifier()
{
    ...
    // aby bardzo du¿y delay by³ naprawdê bardzo du¿y:
    if (this->delay > this->speedModifier->getMaxValue() * 0.9f)
    {
        // ale nie w pierwszym kroku, bo to wygl¹da³oby jak lagi podczas próby odpalenia wizualizacji:
...
{
    // najpierw aktualizujemy przyciski:
    for (auto& it : this->leftSwitches)
    ...
    // sprawdzamy czy zosta³y klikniête podczas ostatniej klatki:
    ...
    // przycisk aktualnego sortowania nie mo¿e zostaæ wy³¹czony:
    if (this->rightSwitches[this->set->getSortType()]->getChange())
        this->rightSwitches[this->set->getSortType()]->setOn(true);

    // ale mo¿e zostaæ w³¹czony inny, wtedy zmieniamy sortowanie:
    for (auto& it : this->rightSwitches)
...
void App::updateSorting()
{
    if (this->timer >= this->delay)
    {
        ...
        {
            // aby przyspieszyæ sortowanie wykonujemy 2 kroki na klatkê:
            this->set->update();
            this->set->update();
        }
